module.exports = {
  getCommandLineHelp: function (command, requiredArgs, optionalArgs) {
    ...
    requiredArgs.forEach(function (argument) {
      output += "  <" + argument.arg + ">" +
        Array(20 - argument.arg.length).join(" ") + argument.desc + "\n";
    });
    output += "\nOptions:\n";
  ...
  mapCommandLineArgs: function (requiredArgs, optionalArgs) {
    ...
    for (r = 0; r < requiredArgs.length; r += 1) {
      if (
        a < process.argv.length && process.argv[a].substr(0, 1) !== "-" &&
        process.argv[a] !== "JASMINE_CONFIG_PATH=./jasmine.json"
      ) {
    ...
    for (; a < process.argv.length; a += 1) {
      for (o = 0; o < optionalArgs.length; o += 1) {
        if (
          optionalArgs[o].alias === process.argv[a].slice(1) ||
          optionalArgs[o].arg === process.argv[a].slice(2)
        ) {
          break;
        }
      }
        ...
        switch (optionalArgs[o].type) {
          ...
          case "number":
            if (a + 1 < process.argv.length) {
              a += 1;
              options[optionalArgs[o].option] =
                (optionalArgs[o].type === "number"
                  ? Number(process.argv[a])
                  : process.argv[a]);
            }
            break;
          case "flag":
            options[optionalArgs[o].option] = true;
            break;
        }
      }
    }
